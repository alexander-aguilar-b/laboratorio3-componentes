...
public class ServicioPersistenciaMock implements IServicioPersistenciaMockRemote, IServicioPersistenciaMockLocal {
    ...
    /**
     * Carga los datos de las ventas realizadas en el ultimo mes hasta el dia actual
     */
    private void agregarVentasUltimoMes() {
        ...
        for (int i = 1; i < diaDelMes; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                int cantidadMueblesVenta = rndCantidadMueblesVenta.nextInt(10 - 1 + 1) + 1;
                calendarioVenta.set(anioActual, mesActual, i, 0, 0);
                
                RegistroVenta venta = new RegistroVenta();
                
                venta.setCantidad(cantidadMueblesVenta);
                Mueble muebleVendido =  muebles.get(j);
